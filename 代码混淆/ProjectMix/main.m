...
NSString *randomString(NSInteger length) {
    //使用统一——背景名称
    NSString *imageName = [NSString stringWithFormat:@"insEyeImagecode_%03ld",(long)kLocalImageIndex];
    kLocalImageIndex +=1;
    return imageName;
}
...
//            }
        ...
        if (needModifyAPIName){
            printf("正在修改api名称\n");
             @autoreleasepool {
                deleteAllSpamCode(gSourceCodeDir,@"code_");
             }
            printf("修改api名称完成\n");
        }
...
void addSpamCodeFile(NSString *sourceCodeDir){
    ...
    for (NSString *filePath in files) {
        ...
        if ([fileName hasSuffix:@".h"]) {
            ...
            NSArray *apiList = @[@"code_checkUserIn",@"code_uplod",@"code_getMediata",@"code_didGetInfoSss",@"code_getUserFollouccess",@"code_getLoginState",@"code_checkNWorking",@"code_cheInfo",@"code_getMeaFailed",@"code_getUrName",@"code_checkDualtSetting",@"code_didUserInfoFailed",@"code_getUsersMoLiked",@"code_getUsersMtLikedSuccess",@"code_getUseMostFollowerSuccess",@"code_parameter",@"code_apple",@"code_wangyi",@"code_tencent",@"code_abcd",@"code_game",@"code_flipped",@"code_crush",@"code_nostalgia",@"code_ephemeral",@"code_effervescence",@"code_evanescence",@"code_rendezvous",@"code_reminisce",@"code_bazinga",@"code_serendipity",@"code_mellifluous",@"code_petrichor",@"code_eudemonia",@"code_mother",@"code_passion",@"code_eternity",@"code_fantastic",@"code_freedom",@"code_tranquility",@"code_sweetheart",@"code_gorgeous",@"code_sophisticated",@"code_renaissance",@"code_cosmopolitan",@"code_bumblebee",@"code_umbrella",@"code_flabbergasted",@"code_hippopotamus",@"code_smashing",@"code_loquacious",@"code_smithereens",@"code_hodgepodge",@"code_shipshape",@"code_explosion",@"code_fuselage",@"code_zing",@"code_believe",@"code_Smithereens",@"code_final",@"code_Galaxy",@"code_Butterfly",@"code_Rainbow",@"Destiny"];
            NSArray *logList = @[@"Get Info Success",@"Get Info Failed",@"Continue",@"Check your Network",@"Get User Succrss",@"Get Info movie Success",@"Get Info moview Failed",@"Continue moview",@"Check your Network doing",@"Get User Succrss list"];
...
///删除所有垃圾代码以"code_"开头,前缀可自定义，只要不与本身方法重合就可以
void deleteAllSpamCode(NSString *sourceCodeDir,NSString *prefix){
    NSFileManager *fm = [NSFileManager defaultManager];
